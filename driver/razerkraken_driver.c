...
/*
 * Work handler for battery_requery_work — scheduled from the cls=0x20
 * RF-reconnect push handler (softirq/URB-completion context, can't sleep or
 * take dev->lock itself). Mirrors exactly what probe() does to prime the
 * cache initially: replay the handshake, then a locked battery query.
 */
static void razer_blackshark_v3_battery_requery(struct work_struct *work)
{
    struct razer_kraken_device *dev =
        container_of(work, struct razer_kraken_device, battery_requery_work);
...
static void razer_blackshark_v3_cache(struct razer_kraken_device *device, u8 *data, int size)
{
    ...
    if ((data[11] == 0x01 || data[11] == 0x02) && data[12] >= 1) {
        ...
        case 0x20: /* RF link re-established — cached battery/charging from
                    * before the drop may be stale, so invalidate them and
                    * kick off a fresh battery query. Without this, battery
                    * stays stuck at -1 forever after the headset reconnects
                    * from being powered off/idle/out of range, since nothing
                    * else re-queries it. schedule_work() is safe to call
                    * from this softirq/URB-completion context (it only
                    * queues work, never sleeps), and is a no-op if
                    * battery_requery_work is already pending.
                    *
                    * V3 Pro only: the requery work's handshake + battery
                    * query is V3 Pro-specific, and plain V3 firmware drops
                    * its RF link on a repeated battery GET (see
                    * razer_blackshark_is_v3pro callers elsewhere) — don't
                    * schedule it for plain V3. */
            if (data[13] == 0x01) {
                device->pushed_charging = -1;
                device->pushed_battery_pct = -1;
                if (razer_blackshark_is_v3pro(device->usb_pid))
                    schedule_work(&device->battery_requery_work);
            }
            break;
        ...
        struct usb_device *usb_dev)
{
    ...
    INIT_WORK(&dev->battery_requery_work, razer_blackshark_v3_battery_requery);
    atomic_set(&dev->intr_eproto_count, 0);
...
exit_free:
    ...
    if (razer_blackshark_is_v3pro(dev->usb_pid)) {
        ...
        cancel_work_sync(&dev->battery_requery_work);
        if (dev->intr_urb) {
            usb_kill_urb(dev->intr_urb);
