...
/* Byte [3] verified 2026-08-07 against a genuine Windows Synapse capture
 * (all five factory presets, software-side preset clicks, EP 0x84 replies
 * kept): every real switch sends 0x01 here, never the 0x00 this table had.
 * That flag is almost certainly "activate this slot now" vs "just store its
 * data" — its absence is the likely root cause of the long-standing bug
 * where our EQ writes ack cleanly but the headset's active preset never
 * moves. See CLAUDE_NOTES.md "EQ preset switch doesn't reach the headset". */
...
/* Esports (slot 4) row corrected 2026-08-07: the real capture's 0x01 0x01 0x01
 * run starts one byte later than the other four rows (offset 4, not 3), with
 * a 0x00 at offset 3 — not the same shape scaled up. Slots 0-3 confirmed
 * byte-for-byte against the same capture; unchanged. */
